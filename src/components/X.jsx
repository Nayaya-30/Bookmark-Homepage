...
const X = () => {
    ...
    return (
        <section className={'flex bg-white flex-col items-center overflow-hidden justify-center gap-6 py-20 px-110'}>
            <h2 className={'text-3xl text-blue-950'}>Frequently Asked Questions</h2>
            ...
            <article className={'relative flex flex-col items-center overflow-hidden justify-center'}>
                {faq.map((item, index) => (
                    <Question click={handleIsOpen} isOpen={isOpen} question={item.question} answer={item.answer} key={index} />
                ))}
            </article>
...
const Question = ({ question, answer, click, isOpen }) => {
    return (
        <>
            <hr className={'h-0.5 w-full'} />
            <div className={'flex justify-between mb-4 mt-2 z-10 bg-white overflow-hidden w-120 items-center pr-4'}>
            ...
            <p className={`text-sm overflow-hidden text-left ${isOpen ? 'open' : 'close'}`}>{answer}</p>
        </>
    )
}
export default X;